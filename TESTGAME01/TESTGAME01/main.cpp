...
vector <Point> body;
vector <Point> foods;
const int FOOD_COUNT = 5; // số miếng food muốn hiển thị cùng lúc
//nextstate
...
{   menuBackground = loadTexture("img/menu_bg.png",gRenderer);
    ...
    //RenderText("DRAGON WORLD", (SCREEN_WIDTH - 200) / 2, menuY - spacing * 2, white);
    int mouseX, mouseY;
...
void HandlePauseInput(SDL_Event& e) {
    if (e.type == SDL_KEYDOWN && (e.key.keysym.sym == SDLK_p || e.key.keysym.sym == SDLK_ESCAPE)) {
        ...
        if (SDL_PointInRect(&mousePoint, &resumeRect)) {
            ...
            highScore = max(highScore, score); SaveHighScore();
            currentState = MENU;
...
if (it != foods.end()) {
    ...
    // giữ nguyên body (ăn rồi nên không pop_back)
} else {
    body.pop_back();
}
             ...
             if (currentTime - lastShrinkTime >= 5000) {
        if (body.size() > 1) {
            body.pop_back();
        }
