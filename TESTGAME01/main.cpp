...
bool   showGood   = false;
Uint32 doubleGood  = 0;
const Uint32 Good_DURATION = 1000;
...
Mix_Chunk* gOhMyGod = nullptr;
Mix_Chunk* gCorrect = nullptr;
...
void RenderGameScreen(const Point& current_direction) {
    ...
    Uint32 now = SDL_GetTicks();
    SDL_Color yellow = {255, 255, 0, 255};
    RenderText("BREAK RECORD!",
               SCREEN_WIDTH/3-80, SCREEN_HEIGHT/3-40,
               yellow, gFont, gRenderer);
    if (now - doubleStart >  Good_DURATION) {
        showGood = false;
    }
}
...
if (it != foods.end()) {
    ...
    if ( !foods.empty() && next_head == foods.back() && score % 3 == 0 && score>0 && score%15 != 0&& score%21 != 0) {
        if(gBomb) play(gBomb);
        currentState = MENU;
    }
    else if(!foods.empty() && next_head == foods.back() && score % 7 == 0 && score>0 )
    {
        ...
        currentState = MENU;}
        else  if(res == 1){score *= 2;
        if(gCorrect) play(gCorrect);
        showDouble  = true;
        doubleStart = SDL_GetTicks();}
    else if(res == 2){ score = highScore +1;
        showGood  = true;
        if(gOhMyGod) play(gOhMyGod);
        doubleStart = SDL_GetTicks();}}
    else score++;
    if (gEat) play(gEat);
    foods.erase(it);
...
int main (int argc, char* argv[]) {
    ...
    if (!gWindow) { cerr << "SDL_CreateWindow Error: " << SDL_GetError() << endl; quitSDL(gEat, gHeadTexture,gBodyTexture, gFoodTexture, gBackgroundTexture, gLose, gFont, gMusic, gRenderer,gWindow); return 1; }
    gRenderer = SDL_CreateRenderer(gWindow, -1, SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC);
    if (!gRenderer) { cerr << "SDL_CreateRenderer Error: " << SDL_GetError() << endl;quitSDL(gEat, gHeadTexture,gBodyTexture, gFoodTexture, gBackgroundTexture, gLose, gFont, gMusic, gRenderer,gWindow); return 1; }
    SDL_SetHint(SDL_HINT_RENDER_SCALE_QUALITY, "1");
    gBigFont = loadFont("font/timesbd.ttf",72);
    gFont = loadFont("font/timesbd.ttf", 36);
    if (!gFont) { cerr << "Failed to load font!" << endl; quitSDL(gEat, gHeadTexture,gBodyTexture, gFoodTexture, gBackgroundTexture, gLose, gFont, gMusic, gRenderer,gWindow); return 1; }
    gBomb = loadSound("audio/bomb.wav");
    gMusic = loadMusic("audio/RunningAway.mp3");
    gEat = loadSound("audio/eating.wav");
    gOhMyGod = loadSound("audio/ohmygod.mp3");
    gCorrect = loadSound("audio/correct.wav");
    ...
    cout<<highScore;
    CoreGame();
   quitSDL(gEat, gHeadTexture,gBodyTexture, gFoodTexture, gBackgroundTexture, gLose, gFont, gMusic, gRenderer,gWindow);
    return 0;
}
