...
// --- Giả sử các hàm tiện ích này được định nghĩa ---
...
void generateFood() {
    bool onBody;
    int maxGridX = SCREEN_WIDTH / RECT_SIZE;
    int maxGridY = SCREEN_HEIGHT / RECT_SIZE;
    int attempts = 0;
    const int MAX_ATTEMPTS = (maxGridX * maxGridY) * 2;

    if (maxGridX <= 0 || maxGridY <= 0) { food = {-RECT_SIZE, -RECT_SIZE}; return; }
    do {
        onBody = false;
        int food_X = rand() % maxGridX;
        int food_Y = rand() % maxGridY;
        food = {food_X * RECT_SIZE, food_Y * RECT_SIZE};
        for (const auto& p : body) { if (food == p) { onBody = true; break; } }
        attempts++;
        if(attempts > MAX_ATTEMPTS) { food = {-RECT_SIZE, -RECT_SIZE}; break; }
    } while (onBody);
}
